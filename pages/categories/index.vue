<template>
  <div>
    ...
    <v-layout row wrap>
      <v-flex xs12 sm4 lg3 v-for="item in categories" :key="item.id">
    ...
    <v-layout row wrap>
      <v-flex v-for="item in adverts" xs12 :sm6="!!item.image" :sm4="!item.image" :key="item.id">
      ...
      <v-flex xs12 sm12 v-if="paginator && paginator.cursor && paginator.cursor.nextURL">
        <div class="my-5 text-xs-center">
          <v-btn round large outline color="info" @click="loadMore" :loading="btnLoading">
...
<script>
  ...
  const title = 'لیست آگهی ها', breadcrumb = 'کل آگهی ها'
